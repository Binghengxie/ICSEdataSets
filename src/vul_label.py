...
def auto_label_devign():
    ...
    TODO: generate bc
    """
    logger.info("start -- label for devign.")
    out_root = "data/outputs"
    # project = "FFmpeg"
    raw_json = "data/devign/function.json"
    done_idxes_path = "data/devign/done.txt"
    # load processed ids
    ...
    logger.info(f"Done load raw label data: {raw_json}")
    ct = 0
    for idx, sample in enumerate(raw_label):
        ...
        project_root = join("data/projects", project)
        code = sample["func"]
            ...
            if sample["target"] == 0:
                ...
                for modi_file in commit.modified_files:
                    if (len(modi_file.methods_before) == 0):
                        logger.info("no method in vul version!")
                        continue
                    if (len(modi_file.methods) == 0):
                        logger.info("no method in safe version!")
                        continue
                ...
                logger.info(f"Done writing: {commit_root}/label.json")
            else:
                # ct = ct - 1
                commit_id = sample["commit_id"]
        ...
        with open(done_idxes_path, "a") as f:
            f.write(str(idx) + ",")
    logger.info(f"end -- label for devign. passed {ct}/{len(raw_label)}")
...
def auto_label_fan():
    ...
    TODO: generate bc
    """
    logger.info("start -- label for Fan.")
    src_root = "data/projects"
    out_root = "data/outputs"
    project = "samba"
    raw_json = "data/fan/MSR_data_simple.json"
    done_idxes_path = "data/fan/done.txt"
    # load processed ids
    if not exists(done_idxes_path):
        os.system(f"touch {done_idxes_path}")
    with open(done_idxes_path, "r") as f:
    ...
    for idx in raw_label:
        ...
        with open(done_idxes_path, "a") as f:
            f.write(str(idx) + ",")
    logger.info(f"end -- label for Fan. passed {ct}/{len(raw_label)}")


def mv_joern_to_outputs(root: str, out_root: str):

    commitids = os.listdir(root)
    for commitid in commitids:
        dirs = join(join(root, commitid), "files")
        files_json_path = join(join(out_root, commitid), "files.json")
        with open(files_json_path, "r") as f:
            files_json = json.load(f)
        files = []
        for ff in files_json:
            files.append(ff)
        # files = [
        #     d for d in os.listdir(dirs)
        #     if ((d != "nodes.csv") and (d != "edges.csv"))
        # ]
        for f in files:
            if (not exists(join(join(out_root, commitid), 'graphs'))):
                # os.system(f"rm -r {join(join(out_root, commitid), 'graphs')}")
                os.mkdir(join(join(out_root, commitid), 'graphs'))
            if (not exists(
                    join(join(join(out_root, commitid), 'graphs'),
                         dirname(f)))):
                print(join(join(join(out_root, commitid), 'graphs'), dirname(f)))
                os.makedirs(
                    join(join(join(out_root, commitid), 'graphs'), dirname(f)))
            print(
                f"cp -r {join(dirs,f)} {join(join(join(out_root, commitid), 'graphs'), dirname(f))}"
            )
            os.system(
                f"cp -r {join(dirs,f)} {join(join(join(out_root, commitid), 'graphs'), dirname(f))}"
            )
...
if __name__ == "__main__":
    auto_label_fan()
    # mv_joern_to_outputs(
    #     "data/outputs/outs/home/chengxiao/project/ICSEDataSets/data/outputs/nginx/",
    #     "data/outputs/nginx")
    # auto_label_devign()

...
from src.vul_label import match_method_to_codelines, build_name_to_method, is_cpp, find_vul_methods
from git import Repo, Git
...
def auto_label1_d2a(project: str):
    """
    generate label for d2a dataset auto labeler 1
    TODO: generate bc
    """
    logger.info(f"start -- Auto Labeler 1 for {project}.")
    labeler_1_file_path = f"data/d2a/{project}/{project}_labeler_1.pickle.gz"
    if not exists(labeler_1_file_path):
        logger.error(f"{labeler_1_file_path} not exists!")
        return
    raw_label_1 = read_pkl(labeler_1_file_path)
    logger.info(f"Done load raw label data: {labeler_1_file_path}")

    out_root = "data/outputs"
    done_idxes_path = f"data/d2a/{project}/done.txt"
    ...
    for sample in raw_label_1:
        ...
        repo, err = checkout_to(project_root, commit_id)
        if (err):
        ...
        if exists(f"{out_root}/{project}/{commit_id}/label.json"):
            logger.info(f"Exists: {out_root}/{project}/{commit_id}/label.json")
...
def auto_label0_d2a(project: str):
    """
    generate label for d2a dataset auto labeler 0
    TODO: generate bc
    """
    logger.info(f"start -- Auto Labeler 0 for {project}.")
    labeler_0_file_path = f"data/d2a/{project}/{project}_labeler_0.pickle.gz"
    if not exists(labeler_0_file_path):
        logger.error(f"{labeler_0_file_path} not exists!")
        return
    raw_label_0 = read_pkl(labeler_0_file_path)
    logger.info(f"Done load raw label data: {labeler_0_file_path}")
    ...
    for sample in raw_label_0:
        ...
        for func_key in funcs:
            ...
            file_path = funcs[func_key]["file"]
            method_name = funcs[func_key]["name"]
            logger.info(f"find safe method: {file_path}/{method_name}")
            spts = funcs[func_key]["loc"].split("-")
            start_line, end_line = int(spts[0].split(":")[0]), int(
                spts[1].split(":")[0])
            write_source_code(file_path, commit_root, project_root)
...
def after_fix_d2a(project: str):
    ...
    TODO: generate bc
    """
    logger.info(f"start -- After Fix 0 for {project}.")
    after_fix_file_path = f"data/d2a/{project}/{project}_after_fix_extractor_0.pickle.gz"
    if not exists(after_fix_file_path):
        logger.error(f"{after_fix_file_path} not exists!")
        return
    after_fix_label = read_pkl(after_fix_file_path)
    logger.info(f"Done load raw label data: {after_fix_file_path}")
